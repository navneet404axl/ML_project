...
\item \textbf{Sparse Rule Selection:} We frame rule selection as a sparse linear regression problem. A binary indicator matrix encodes which rules apply to which training instances, and $L_1$ regularization (Lasso) is applied to automatically zero out redundant and uninformative rules. The surviving non-zero rules form our final interpretable rule set. We additionally implement SIRUS \cite{benard2021sirus} as a frequency-based alternative for comparison.
\end{enumerate}
...
We evaluate on two complementary clinical classification datasets. The first is the Breast Cancer Wisconsin (Diagnostic) dataset, available directly through scikit-learn, which contains 569 patient records each described by 30 numerical features extracted from fine needle aspirate biopsy images. The target variable is binary: malignant (212 patients) or benign (357 patients). The second is the Pima Indians Diabetes dataset, containing 768 patient records described by 8 features (pregnancies, glucose, blood pressure, skin thickness, insulin, BMI, diabetes pedigree function, and age) with a binary diabetic / non-diabetic outcome (268 / 500 patients).
...
To establish a baseline, we trained a Random Forest of 100 decision trees, each with a maximum depth of 5, on the training split of each dataset. This gives us a strong, accurate reference model whose internal structure we will distill in the subsequent steps. We evaluated each forest on the held-out test set and recorded accuracy, F1 score, and a full classification report so we have a concrete performance ceiling to measure against later. We also pulled the Gini feature importances out of each trained forest to use later as a reference point for validating that our extracted rules identify the same signals the forest itself considered important.

We use Random Forests because they work well for tabular clinical data and can capture nonlinear relationships between features. For example, diabetes risk may depend on combinations of glucose, BMI, age, and insulin rather than on one variable by itself. A Random Forest can model these interactions without requiring us to assume a simple linear relationship in the original feature space. At the same time, the model is still built from decision trees, which makes it possible to extract rule paths afterward. We limit each tree to depth 5 so that the extracted rules do not become too long or too specific to a small number of training examples.
...
We formulate rule selection as a sparse linear regression problem. Given the binary indicator matrix $A$ and the true class labels $y$, we solve the following optimization objective:
\begin{equation}
\min_{\alpha} \frac{1}{2N} \| y - A\alpha \|_2^2 + \lambda \| \alpha \|_1
\end{equation}
The $L_1$ penalty forces the coefficients of redundant, noisy, or highly correlated rules to exactly zero. Only rules with non-zero coefficients after optimization survive as our final sparse rule set. We use \texttt{LassoCV} with 5-fold cross-validation to select an initial regularization strength $\lambda$, then sweep $\lambda$ across multipliers (1$\times$, 2$\times$, 5$\times$, 10$\times$, 20$\times$, 30$\times$ the cross-validated value) to study the tradeoff between number of surviving rules and predictive accuracy. This frames the problem as a rule ensemble \cite{friedman2008rule}.
...
As a second rule-extraction method we implemented SIRUS \cite{benard2021sirus}. The pipeline differs from the Lasso path at every step:
...
We include SIRUS because it gives us a second way to think about what makes a rule useful. Lasso keeps rules that help the sparse predictive model, while SIRUS keeps rules that appear repeatedly across many shallow trees. In other words, Lasso focuses more on predictive contribution, and SIRUS focuses more on stability and simplicity. Comparing both methods lets us see whether the same interpretability story holds under two different rule-selection assumptions.
...
We use a held-out test set to measure performance on examples the model has not seen before. This is important for our project because the pipeline creates thousands of possible rules from the training forest. Some of those rules may describe real patterns, but others may only fit small quirks of the training data. If we evaluated the final rule set on the same data used to train the forest and choose the rules, the results would likely look better than they really are.

We use stratified sampling for the 80\%/20\% train-test split. In practice, this means the split uses the class label, so the malignant/benign ratio in Breast Cancer and the diabetic/non-diabetic ratio in Pima stay roughly the same in both training and test data. This matters especially for Pima because diabetic patients are the minority class. Without stratification, the test set could end up with too few diabetic patients, making diabetic recall unstable or misleading.

We also use \texttt{LassoCV} instead of choosing the regularization strength by hand. This lets the model choose a reasonable penalty through cross-validation on the training data before we evaluate on the held-out test set. The $L_1$ penalty helps control overfitting by setting many weak rule weights to zero. In our setting this is important because the forest produces thousands of possible rules, and many of them may only describe very specific training examples rather than patterns that generalize. The final rule set is therefore the smaller group of rules that still carries useful predictive signal.

\subsection{Evaluation Metrics and Success Criteria}

We evaluate the system across three dimensions:

\textbf{Predictive Fidelity:} accuracy and F1 score of the sparse rule set versus the Random Forest baseline on the held-out test set. Accuracy tells us the overall percentage of correct predictions, while F1 gives a better sense of the balance between precision and recall for the positive class. We also report per-class recall because clinical errors are not all equally serious. On the Pima dataset, missing diabetic patients is more concerning than simply maximizing total accuracy, so recall helps reveal class-imbalance failures that accuracy alone can hide.
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{images/tradeoff_curve.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{images/sirus_pima_tradeoff.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{images/sirus_confusion_matrices.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{images/pima_tradeoff_curve.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{images/pima_confusion_matrices.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{images/signal_vs_noise.png}
...
\bibitem{friedman2008rule}
Jerome H. Friedman and Bogdan E. Popescu. 2008. Predictive learning via rule ensembles. \textit{The Annals of Applied Statistics}, 2(3):916--954.

\bibitem{benard2021sirus}
Cl\'ement B\'enard, G\'erard Biau, S\'ebastien da Veiga, and Erwan Scornet. 2021. SIRUS: Stable and Interpretable RUle Set for Classification. \textit{arXiv preprint arXiv:1908.06852}.
